...
add_hook('InvoicePaid', 2, function($vars) {
    ...
    if($result_invoice['total'] > 10 and $client['currency_code'] == 'CLP' and $client['countrycode'] == 'CL'  and $result_invoice['tax'] > 0 ){
        ...
        if(isset($openfactura_config[0])){
            // Check if invoice URL was already generated
            $registry = Capsule::table('mod_openfactura_registry')->where('apikey', $openfactura_config[0]->apikey)
                ->where('invoice_id', $invoiceid)
                ->get();

            if (isset($registry[0])) {
                logModuleCall('openfactura', 'InvoicePaid hook', ["invoiceid" => $invoiceid], "The URL for this invoice was already generated" , ["message" => "The URL for this invoice was already generated"], []);
                return;
            }
