...
class Controller
{
    ...
    public function index($vars, $params)
    {
        ...
        if (!empty($urlParams['data'])) {
            ...
            $smarty->assign('RootDirectory', $CONFIG['SystemURL']);
    ...
    public function registration($vars, $params)
    {
        ...
        foreach ($openfactura_registry as $key => $value) {
            $api_results = localAPI('GetInvoice', ['invoiceid' => $value->invoice_id]);
            ...
            array_push($registry, [
                ...
                "whmcs_invoice_status"=> $api_results["status"],
            ]);
        }
        ...
        if (!empty($invoiceid)) {
            ...
            if (isset($invoicewhmcs[0])) {
                ...
                if (isset($invoiceof[0])) {
                    ...
                    //erase condition  and $result_invoice['status']=='Paid'
                    if($result_invoice['total'] > 10 and $client['currency_code'] == 'CLP' and $client['countrycode'] == 'CL'  and $result_invoice['tax'] > 0){
                        ...
                        if($result_invoice['tax'] <= 0){
                            ...
                            logModuleCall('openfactura', 'El invoice que desea agregar debe tener un impuesto mayor a 0: ' . $invoiceid, json_encode($json, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES));
                        }
                    }
                }
            } else {
